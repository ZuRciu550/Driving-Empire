...
-- Uçuşu güvenli bir şekilde durdurmak (Yürümeye geçmeden ve yere basmadan önce)
...
-- Hızı ayarlanabilir, Noclip açık uçuş
local function flyToPos(targetPos, speed)
	...
	-- Uçarken binalara takılmamak için Noclip
	...
	currentFlightTween.Completed:Wait()
	hrp.Velocity = Vector3.zero
end
...
local function getApproachPos(hrpPos, targetPos, distance)
	...
	if offset.Magnitude < 0.1 then 
		...
		offset = offset.Unit * distance
	end
	-- Yükseklik 10 olarak ayarlandı (Yürümeden önce 10 stud yukardan gelir)
...
local function getModelEdgeApproachPos(hrpPos, model)
	...
	local dir = Vector3.new(center.X - hrpPos.X, 0, center.Z - hrpPos.Z)
...
DeliveryTab:CreateToggle({
	...
	Callback = function(val)
		...
		if val then
			...
			task.spawn(function()
				while autoDelivery and currentLoopId == thisLoopId do
					local char = LocalPlayer.Character
					local hrp = char and char:FindFirstChild("HumanoidRootPart")
					if not hrp then task.wait(1) continue end

					if deliveryTag == "Baslat" then
						print("[Auto Delivery Log] Görev noktasına yaklaşılıyor...")
						...
						stopFlight() 
						task.wait(0.4) -- 10 studdan yere düşmesi için bekleme payı
						
						-- Son 10 studu yürüyerek git
						...
						if plrGui then
							local promptUI = plrGui:WaitForChild("PromptUI", 3)
							if promptUI then
								local promptV2 = promptUI:WaitForChild("PromptV2", 3)
								if promptV2 then
									local buttonsFrame = promptV2:WaitForChild("ButtonsFrame", 3)
									if buttonsFrame then
										local highRiskBtn = buttonsFrame:WaitForChild("HighRiskDelivery", 3)
										if highRiskBtn then
											pcall(function()
												for _, conn in pairs(getconnections(highRiskBtn.MouseButton1Click)) do conn:Fire() end
												for _, conn in pairs(getconnections(highRiskBtn.Activated)) do conn:Fire() end
												for _, conn in pairs(getconnections(highRiskBtn.MouseButton1Down)) do conn:Fire() end
											end)
											
											local absPos = highRiskBtn.AbsolutePosition
											local absSize = highRiskBtn.AbsoluteSize
						...
						task.wait(1.5)
						deliveryTag = "Toplayan"

					elseif deliveryTag == "Toplayan" or deliveryTag == "Veren" then
						print("[Auto Delivery Log] Tag Aktif: " .. deliveryTag)
						
						local effectModel = workspace:FindFirstChild("DeliveryLocationEffects")
						
						if not effectModel then
							local anchor = workspace:FindFirstChild("DeliveryTargetAnchor", true)
							if anchor then
								flyToPos(anchor.Position + Vector3.new(0, 15, 0), 2000)
								if currentLoopId ~= thisLoopId then break end
								
								local waitTimeout = 0
								while not workspace:FindFirstChild("DeliveryLocationEffects") and waitTimeout < 50 do
									task.wait(0.1)
									waitTimeout = waitTimeout + 1
								end
								effectModel = workspace:FindFirstChild("DeliveryLocationEffects")
							end
						end
						
						if effectModel then
							-- Hitbox'ın sınırından 10 stud uzağı ve 10 stud yukarısını hesapla
							...
							stopFlight() -- Serbest bırak
							task.wait(0.4) -- 10 stud yukardan yere düşmesini bekle
							
							-- Hitbox'ın merkezine yürü
							walkToPos(centerPos, thisLoopId)
							if currentLoopId ~= thisLoopId then break end
							
							-- İstenen bekleme süreleri (Toplayan: 6, Veren: 0.3)
